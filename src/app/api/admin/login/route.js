...
export async function POST(request) {
  try {
    ...
    let passwordHash = rawHash.trim().replace(/^['"]|['"]$/g, "").replace(/\\\\/g, "");
    
    // Auto-decode if it's Base64 (hashes always start with $2b$, so if it doesn't, it might be encoded)
    if (!passwordHash.startsWith("$2b$") && passwordHash.length > 30) {
      try {
        const decoded = Buffer.from(passwordHash, "base64").toString("utf8");
